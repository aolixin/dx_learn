...
bool SkyboxEffect::InitAll(ID3D11Device* device)
{
    ...
    // 创建顶点着色器
    HR(pImpl->m_pEffectHelper->CreateShaderFromFile("SkyboxVS", L"ShaderBin/Skybox_VS.cso",
    ...
    // 创建像素着色器
    HR(pImpl->m_pEffectHelper->CreateShaderFromFile("SkyboxPS", L"ShaderBin/Skybox_PS.cso", device));
