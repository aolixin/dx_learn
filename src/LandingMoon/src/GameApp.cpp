...
bool GameApp::InitResource()
{
    ...
    // 初始化地板
    Model* pModel = m_ModelManager.CreateFromFile("Resource\\Models\\ground_19.obj");
    m_Ground.SetModel(pModel);
    pModel->SetDebugObjectName("ground_19");


    // 初始化房屋模型
    pModel = m_ModelManager.CreateFromFile("Resource\\Models\\sun.obj");
    m_sun.SetModel(pModel);
    pModel->SetDebugObjectName("sun");

    // 获取房屋包围盒
    XMMATRIX S = XMMatrixScaling(0.015f, 0.015f, 0.015f);
    BoundingBox houseBox = m_sun.GetModel()->boundingbox;
    houseBox.Transform(houseBox, S);
    // 让房屋底部紧贴地面
    Transform& houseTransform = m_sun.GetTransform();
